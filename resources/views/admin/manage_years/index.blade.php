...
@section('content')
    ...
    <title>Year Index</title>


    <div class="box" style="align-items: stretch">
        <div class="box-header">
            <p class="pull-left">
                <a href="{{route('years.create')}}" style="margin-left: 50px" class="btn btn-success waves-effect waves-light form-control pull-right" style="float: none;margin: 5px;">
                    New Year</a>
            </p>
        </div>
                ...
                @foreach($years as $u)
                    <tr>
                        ...
                        <td class="tabledit-toolbar-column" style="text-align: center;">
                            <a href="{{route('years.edit', $u->id)}}">
                                <form action="{{ route('years.edit',  $u->id)}}" class="tabledit-edit-button btn btn primary waves-effect waves-light">
                                    @csrf
                                    @method('GET')
                            ...
                            <a href="{{route("years.destroy",$u->id)}}"  onclick="return confirm('Are you sure you want to delete?')">
                                <form action="{{route("years.destroy",$u->id)}}" method="post" class="tabledit-edit-button btn btn primary waves-effect waves-light float-left">
                                    @csrf
                                    @method('DELETE')
